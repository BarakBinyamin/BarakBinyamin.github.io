<template>
    <div v-for="blog in blogs" class="post-metadata">
      <div lang="de" class="post-title">{{ blog.meta?.title }}</div>
      <div lang="de" class="post-subtitle">{{ blog.meta?.subtitle }}</div>
      <div class="post-info"> 
  ...
  export default{
    ...
    methods: {
      async load(){
        ...
        const converter   = await (new showdown.Converter(options))
        await converter.setFlavor('github')
